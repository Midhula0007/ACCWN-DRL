...
PrintRxCount()
{
  ...
  //myfile<<"received pkt size : "<<size<<endl;
...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("nRLeaf",     "Number of right side leaf nodes", nRLeaf);
  ...
  */
  ...
  myfile<<"IP Header size is: " << ip_header<<endl;
  delete temp_header;
  temp_header = new TcpHeader ();
  uint32_t tcp_header = temp_header->GetSerializedSize ();
  NS_LOG_LOGIC ("TCP Header size is: " << tcp_header);
  myfile<<"TCP Header size is: " << tcp_header<<endl;
  delete temp_header;
  uint32_t tcp_adu_size = mtu_bytes - 20 - (ip_header + tcp_header);
  NS_LOG_LOGIC ("TCP ADU size is: " << tcp_adu_size);
  myfile<<"TCP ADU size is: " << tcp_adu_size<<endl;
  myfile<<"mtu_bytes :"<<mtu_bytes<<endl;
  ...
  PointToPointDumbbellHelper d (nLeaf, pointToPointLeaf,
                                nRLeaf, pointToPointLeaf,
                                bottleNeckLink);
  
  // Wireless

  NodeContainer wifiStaNodes;
  wifiStaNodes.Create (1);   //Midhula
  NodeContainer wifiApNode = d.GetLeft(0);
  ...
  //wifiStaNodes.Add (wifi.Install (phy, mac, NodeContainer(wifiStaNodes.Get (1), wifiStaNodes.Get (2))));
  ...
  NodeContainer enbNodes=d.GetRight(0); //actually 0, now 2 midhula
  NodeContainer ueNodes;
  //enbNodes.Create (2);
  ueNodes.Create (2);  // It was 2. 3 by midhula
  
  // Install Mobility Model
  MobilityHelper mobility_;
  mobility_.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  mobility_.Install (enbNodes);
  BuildingsHelper::Install (enbNodes);
  mobility_.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  mobility_.Install (ueNodes);
 ...
 // myfile<<"Assign IP Addresses"<<endl;
  ...
  myfile<<"Install apps in left and right nodes"<<endl;
  uint16_t port = 50000;
  ...
  for (uint32_t i = 0; i < d.RightCount (); ++i)
  {
    ...
    //myfile<<"protocol : "<<TypeIdValue (TcpSocketFactory::GetTypeId ())<<endl;
    myfile<<"RIGHT node count :"<<d.RightCount ()<<"node in "<<d.GetRight (i)<<"with IP : "<<d.GetRightIpv4Address (i)<<endl;
  }
  ...
  {
    myfile<<"left node count :"<<d.LeftCount ()<<"node in "<<d.GetLeft (i)<<"with IP :"<<d.GetLeftIpv4Address (i)<<endl;
    //myfile<<"Create an on/off app sending packets to the left side"<<endl;
    AddressValue remoteAddress (InetSocketAddress (d.GetRightIpv4Address (i), port));
    myfile<<" right side ip address : "<<d.GetRightIpv4Address (i)<<endl;
    myfile<<" with port : "<<port<<endl;
    Config::SetDefault ("ns3::TcpSocket::SegmentSize", UintegerValue (tcp_adu_size));
    BulkSendHelper ftp ("ns3::TcpSocketFactory", Address ());
    ftp.SetAttribute ("Remote", remoteAddress);
    ftp.SetAttribute ("SendSize", UintegerValue (tcp_adu_size));
    ftp.SetAttribute ("MaxBytes", UintegerValue (int(data_mbytes*1000000)));
    myfile<<"\n MaxBytes :"<< data_mbytes<<endl;
    myfile<<"\n SendSize :"<< tcp_adu_size<<endl;
  ...
  for (uint32_t i = 0; i < d.RightCount (); ++i)
  {
    ...
    //myfile<<"\n RX packet count :"<<CountRxPkts<<endl;
  }
